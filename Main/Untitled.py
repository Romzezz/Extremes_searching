...
def _take_input(ask_restriction=False):
    varnames = list(input(f"Введите названия переменных\nПрим. x y: ").split())
...
def _plot(func, points, bounds=None, restriction=None):
    ...
    if restriction:
        ...
        print(sympy.lambdify(args, restriction)(0, 0))
...
def find_local_extremas(varnames, func, bounds=None):
    ...
    _input = _take_input()
    args = sympy.symbols(_input['varnames'])
    sympifyed = sympy.sympify(_input['func'])
